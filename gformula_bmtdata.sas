...
PROC PHREG DATA = person_day_level;
 TITLE "Crude model";
 MODEL (yesterday day)*d(0) = gvhd / TIES=EFRON RL;
RUN;
PROC PHREG DATA = person_day_level;
 TITLE "Crude model";
 MODEL (yesterday day)*d(0) = gvhd1 gvhd2 / TIES=EFRON RL;
 gvhd1=gvhd*(day<=100); gvhd2=gvhd*(day>100);
RUN;
PROC PHREG DATA = person_day_level;
 TITLE "Baseline adjusted model";
 MODEL (yesterday day)*d(0) =gvhd all age agecurs1 agecurs2 male cmv wait / TIES=EFRON RL;
RUN;
PROC PHREG DATA = person_day_level;
 TITLE "Baseline adjusted model";
 MODEL (yesterday day)*d(0) =gvhd1 gvhd2 all age agecurs1 agecurs2 male cmv wait / TIES=EFRON RL;
 gvhd1=gvhd*(day<=100); gvhd2=gvhd*(day>100);
RUN;
PROC PHREG DATA = person_day_level;
 TITLE "Fully adjusted model";
 MODEL (yesterday day)*d(0) =gvhd relapse platnorm all age agecurs1 agecurs2 male cmv wait / TIES=EFRON RL;
RUN; 
PROC PHREG DATA = person_day_level;
 TITLE "Fully adjusted model";
 MODEL (yesterday day)*d(0) =gvhd1 gvhd2 relapse platnorm all age agecurs1 agecurs2 male cmv wait / TIES=EFRON RL;
 gvhd1=gvhd*(day<=100); gvhd2=gvhd*(day>100);
RUN; 


PROC LIFETEST DATA = person_level METHOD=km NOPRINT OUTSURV=fobssurv(KEEP=t survival);
 TITLE "Observed Product limit survival estimate";
 TIME t*d_dea(0);
RUN;
PROC LIFETEST DATA = natcourse METHOD=km NOPRINT OUTSURV=f0surv(KEEP=td survival);
 TITLE "Natural course limit survival estimate";
 TIME td*d(0);
RUN;
PROC LIFETEST DATA = nevergvhd METHOD=km NOPRINT OUTSURV=f1surv(KEEP=td survival);
 TITLE "Intervention: never experience GvHD - product limit survival estimate";
 TIME td*d(0);
RUN;
PROC LIFETEST DATA = alwaysgvhd METHOD=km NOPRINT OUTSURV=f2surv(KEEP=td survival);
 TITLE "Intervention: experience GvHD from day 1 - product limit survival estimate";
 TIME td*d(0);
RUN;
DATA survplot;
 SET fobssurv(IN=ina RENAME=(t=day)) 
  f0surv(IN=inb RENAME=(td=day)) 
  f1surv(IN=inc RENAME=(td=day)) 
  f2surv(IN=ind RENAME=(td=day));
 IF ina THEN type="Observered    ";
 IF inb THEN type="Natural course";
 IF inc THEN type="Never exposed ";
 IF ind THEN type="Always exposed";

PROC SORT DATA = survplot;
 BY type day;
DATA survplot; 
 SET survplot; 
 BY type day;
...
PROC SGPLOT DATA = survplot;
 TITLE "G formula - survival curves: observed, natural course, interventions";
 STEP X= day Y=survival / GROUP=type;
RUN;

%LET PATH = \\tsclient\Aldrive\Users\akeil\Documents\Papers\2012_G_methods_compare;
%LET dpath = &path.\data;
%LET Gpath = &path.\images;
LIBNAME bmt "&dpath";

DATA bmt.bmt_int_survdata;
 SET survplot;
DATA bmt.bmt_int_alwaysexp;
 SET alwaysgvhd;
DATA bmt.bmt_int_neverexp;
 SET nevergvhd;
DATA bmt.bmt_int_natcours;
 SET natcourse;

PROC EXPORT DATA= survplot 
 OUTFILE= "&dpath./bmt_int_survdata.csv" 
 DBMS=CSV REPLACE;
PROC EXPORT DATA=  alwaysgvhd
 OUTFILE= "&dpath./bmt_int_alwaysexp.csv" 
 DBMS=CSV REPLACE;
PROC EXPORT DATA=  nevergvhd
 OUTFILE= "&dpath./bmt_int_neverexp.csv" 
 DBMS=CSV REPLACE;
PROC EXPORT DATA=  natcourse
 OUTFILE= "&dpath./bmt_int_natcours.csv" 
 DBMS=CSV REPLACE;
DATA obscourse;
 SET person_level(KEEP=id t: d: 
 RENAME=(t=td t_gvhd = tg t_pla = tp t_rel=tr d_dea=d d_gvhd=gvhd d_rel=relapse d_pla=platnorm));
PROC EXPORT DATA=  obscourse           
 OUTFILE= "&dpath./bmt_int_obs.csv" 
 DBMS=CSV REPLACE;
RUN;QUIT;RUN;
*/
